...
BEGIN
    ...
    IF rec.collecting_event_id IS NOT NULL THEN
        ...
        if gcollecting_event_id is not null then
            ...
            --dbms_output.put_line('got gcollecting_event_id @ user picked event name: ' || gcollecting_event_id);
            return;
        else
    ...
    if gLocalityId is null then
        ...
        IF rec.orig_lat_long_units IS NULL THEN
            ...
            WHERE
                ...
                nvl(concatGeologyAttributeDetail(locality.locality_id),'NULL') = nvl(b_concatGeologyAttributeDetail(rec.collection_object_id),'NULL')
            ;
        ELSE          
           select 
                min(locality.locality_id)
