...
collectionid NUMBER;
  ...
  BEGIN
	FOR rec IN (SELECT * FROM bulkloader where loaded is null) LOOP
		...
		SELECT count(*) INTO numRecs FROM accn,trans,collection WHERE 
		accn.transaction_id=trans.transaction_id AND
		trans.collection_id=collection.collection_id AND
		ACCN_NUMBER = tempStr2 AND 
		collection.institution_acronym=tempStr;
...
--	FOR r IN (select container_id from coll_obj_cont_hist where collection_object_id IN (
--				select collection_object_id FROM specimen_part WHERE derived_from_cat_item = l_collection_object_id
--			    )
--			) LOOP
--			delete from container where container_id = r.container_id;
--	END LOOP;
...
BEGIN
        ...
        dbms_output.put_line(someRandomNumber);
	...
	if aRec.locality_id is null then -- otherwise, we already have what we need
		...
		ELSIF aRec.orig_lat_long_units = 'decimal degrees' THEN
            ...
            WHERE
            	...
            	NVL(determined_date,'1-JAN-1600') = NVL(to_date(aRec.determined_date),'1-JAN-1600') AND 
...
l_taxon_name_id_2 := NULL;
    ...
    select count(distinct(accn.transaction_id)) into num from accn,trans,collection where 
    	accn.transaction_id = trans.transaction_id and
    	trans.collection_id=collection.collection_id AND
    	collection.institution_acronym = tempStr and
