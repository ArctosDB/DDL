...
-- procedure to run occasionally
-- takes from taxon_term_updated
-- refreshes flat when something's changed



--- REBUILD THIS
CREATE OR REPLACE PROCEDURE update_flat_taxonomy is
	BEGIN
		...
		update flat set stale_flag=1 where collection_object_id in (
			...
			where
				identification.identification_id=identification_taxonomy.identification_id and
				identification_taxonomy.taxon_name_id=taxon_term_updated.taxon_name_id
		);
