...
CREATE OR REPLACE TRIGGER TR_COLLECTINGEVENT_BUID
	-- formerly TR_COLLECTINGEVENT_BUD and TRG_COLLECTING_EVENT_BIU
	-- bringing everything in to one trigger so we can
	-- more easily pass admin_flag around
	BEFORE UPDATE OR DELETE or INSERT ON collecting_event
	FOR EACH ROW
	DECLARE
	    num NUMBER;
	    allrec INTEGER;
	    ...
	    status varchar2(255);
	BEGIN
	-- flag set by auto-merge scripts
...
CREATE OR REPLACE TRIGGER TR_COLLECTINGEVENT_BUD
	...
	no compile
	merged into TR_COLLECTINGEVENT_BUID
	drop trigger TR_COLLECTINGEVENT_BUD;
...
CREATE OR REPLACE TRIGGER TRG_COLLECTING_EVENT_BIU
	...
	no compile
	merged into TR_COLLECTINGEVENT_BUID
	drop trigger TRG_COLLECTING_EVENT_BIU;
