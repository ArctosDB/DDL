...
PROCEDURE set_ala_status 
	...
	WHERE NOT EXISTS (
		SELECT scientific_name 
			FROM taxonomy 
		WHERE 
			VALID_CATALOG_TERM_FG=1 AND
			scientific_name = ala_plant_imaging.folder_identification
	    ) AND 
	    status = 'processing';	
	...
	UPDATE ala_plant_imaging 
	SET status = 'dup_id_num_in_imager'
	WHERE idnum||idtype  IN (
		SELECT idnum||idtype 
		FROM ala_plant_imaging 
		HAVING count(idnum||idtype) > 1
		GROUP BY idnum||idtype
		) AND 
		status = 'processing';
	
	UPDATE ala_plant_imaging 
	SET status='dup_in_bulkloader'
	where
        idnum||idtype IN (
			SELECT other_id_num_1 || other_id_num_type_1
			FROM bulkloader
			where bulkloader.other_id_num_type_1=ala_plant_imaging.idtype
		)
	    AND 
		status = 'processing';
	...
	UPDATE ala_plant_imaging 
		SET status = 'dup_in_arctos'
	    WHERE idnum||idtype IN (
			SELECT display_value||other_id_type 
			FROM coll_obj_other_id_num
			where coll_obj_other_id_num.other_id_type=ala_plant_imaging.idtype 
		) AND 
		status = 'processing';
