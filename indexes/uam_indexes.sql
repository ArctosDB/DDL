...
CREATE INDEX IX_COLLECTING_EVENT_UPR_VLOC ON COLLECTING_EVENT (upper(verbatim_locality)) TABLESPACE UAM_IDX_1;
...
CREATE UNIQUE INDEX PK_COLL_OBJECT
	...
	-- for growth summary eg /info/sysstats.cfm
	
	create index ix_cobjentyr on coll_object (to_number(to_char(COLL_OBJECT_ENTERED_DATE,'YYYY'))) tablespace uam_idx_1;
...
CREATE INDEX IX_CONTAINER_LABEL
	ON CONTAINER (label)
	TABLESPACE UAM_IDX_1;
...
CREATE UNIQUE INDEX IU_GEOGAUTHREC_HIGHERGEOG
	...
	CREATE INDEX IX_GEOGAUTHREC_upr_HIGHERGEOG ON GEOG_AUTH_REC (upper(HIGHER_GEOG)) TABLESPACE UAM_IDX_1;
...
CREATE INDEX IX_IDENTIFICATION_COLLOBJID
	...
	alter index IX_IDENTIFICATION_COLLOBJID rebuild;
...
CREATE UNIQUE INDEX PK_IDENTIFICATION
	...
	TABLESPACE UAM_IDX_1;
		...
		alter index PK_IDENTIFICATION rebuild;
...
alter index IX_IDENTIFICATIONTAXONOMY_IID rebuild;
...
alter index IX_IDENTIFICATIONTAXONOMY_TNID rebuild;
...
CREATE INDEX IX_LOCALITY_UPR_SPECLOC ON LOCALITY (upper(spec_locality))	TABLESPACE UAM_IDX_1;
CREATE INDEX IX_LOCALITY_UPR_sgeo ON LOCALITY (upper(S$GEOGRAPHY))	TABLESPACE UAM_IDX_1;
CREATE INDEX IX_LOCALITY_UPR_locname ON LOCALITY (upper(LOCALITY_NAME))	TABLESPACE UAM_IDX_1;

                    select locality_id from locality where upper(LOCALITY_NAME) LIKE '%BOREHOLE%'
...
CREATE INDEX IX_SPECPARTATT_ATTTYPE ON SPECIMEN_PART_ATTRIBUTE (ATTRIBUTE_TYPE)	TABLESPACE UAM_IDX_1;
